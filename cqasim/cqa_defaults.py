...
class CqaDefaults:
    ...
    def build_fp(self, base_path):
        ...
        if not self.par:  # TODO: test with simulator
    ...
    def build_fn(self):
        ...
        fn_params = [
            f"fieldM{self.par['M']}",
            f"heightM{self.par['mean_height']}",
            f"diaM{self.par['mean_diameter']}",
            f"diaVar{self.par['var_diameter']}",
            f"corrP{int(self.par['correlated_peaks'])}",
